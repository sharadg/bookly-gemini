...
class BooklyLiveAgent:
    ...
    def __init__(self, *, modality: str = "AUDIO") -> None:
        ...
        # Server-side identity cache: if someone already verified on this
        # server process, hydrate the new Session so they don't re-verify
        # on browser refresh / new tab. See identity_cache.py — the
        # browser never writes to this cache.
        if identity_cache.hydrate(self.session):
            log_event(
                "session_hydrated_from_cache",
                email=self.session.verified_email,
            )
